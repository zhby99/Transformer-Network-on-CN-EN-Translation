    ...
    def __init__(self, is_training=True):
        ...
        with self.graph.as_default():
            ...
            with tf.variable_scope("decoder1"):
                ## Embedding
                self.dec1 = embedding(self.decoder_inputs,
                ...
                ## Positional Encoding
                if hp.sinusoid:
                    self.dec1 += positional_encoding(self.decoder_inputs,
                                      vocab_size=hp.maxlen,
                                      num_units=hp.hidden_units,
                                      zero_pad=False,
                                      scale=False,
                    ...
                    self.dec1 += embedding(tf.tile(tf.expand_dims(tf.range(tf.shape(self.decoder_inputs)[1]), 0), [tf.shape(self.decoder_inputs)[0], 1]),
                                      vocab_size=hp.maxlen,
                                      num_units=hp.hidden_units,
                                      zero_pad=False,
                                      scale=False,
                ...
                ## Dropout
                self.dec1 = tf.layers.dropout(self.dec1,
                                            rate=hp.dropout_rate,
                                            training=tf.convert_to_tensor(is_training))
                ...
                for i in range(hp.num_blocks):
                    with tf.variable_scope("num_blocks_{}".format(i)):
                        ...
                        self.dec1 = multihead_attention(queries=self.dec1,
                                                        keys=self.dec1,
                                                        num_units=hp.hidden_units,
                                                        num_heads=hp.num_heads,
                                                        dropout_rate=hp.dropout_rate,
            ...
            with tf.variable_scope("decoder2"):
                ...
                for i in range(hp.num_blocks):
                    with tf.variable_scope("num_blocks_{}".format(i)):
                        ...
                        self.dec2_1 = tf.scalar_mul(0.9, self.dec2_1)
                        self.dec2_2 = tf.scalar_mul(0.1, self.dec2_2)
                        self.dec2 = tf.add(self.dec2_1, self.dec2_2)
                        ## Feed Forward
